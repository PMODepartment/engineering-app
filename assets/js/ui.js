...
(function () {
  ...
  // ---- Project selector (shared, flat) -------------------------------------
  // Upgrades a native project <select> into a searchable flat list of projects.
  // The <select> stays the source of truth (its value + change events still
  // fire), so existing `sel.onchange` handlers keep working. The list is built
  // from PDb.getProjects, FILTERED to the ids present in the select's options — so any module-level access filtering
  ...
  //
  // ⚠️ There is NO foldering left here. This was a recursive Workspace → Program
  // → Group walk, then a one-level group-head drill-down; it is now a flat
  // searchable list. Do not reintroduce a hierarchy — a project is picked by
  // name, and every folder level was a click between the user and that name.
  var _pdProjCache = null;   // per-page (one load), shared across instances
  function enhanceProjectSelect(sel) {
    ...
    // No foldering. This is a flat, searchable project list — the same shape as
    // the Projects page and the Procurement App's picker. Grouping the switcher
    // by group head meant two clicks and a mental model ("which head owns this?")
    // to reach a project the user could already name.
    function head() {
      return '<div class="pd-pss-search"><input type="text" class="pd-pss-q" ' +
        'placeholder="Search projects…" value="' + esc(search) + '"></div>';
    }
    function render() {
      var ids = {};
      ...
      function projRow(p) {
        return '<div class="pd-pss-proj' + (p.id === sel.value ? ' sel' : '') + '" data-proj="' +
          esc(p.id) + '">' + ico('project', 14) + '<span>' + esc(p.name || p.id) + '</span></div>';
      }
      var q = search.trim().toLowerCase();
      var list = (q ? P.filter(function (p) {
        return (p.name || '').toLowerCase().indexOf(q) !== -1 ||
               (p.id || '').toLowerCase().indexOf(q) !== -1;
      }) : P).sort(byName);
      var body = list.length ? list.map(projRow).join('')
        : '<div class="pd-pss-empty">' + (q ? 'No projects match \u201c' + esc(search) + '\u201d.'
      ...
      if (qi) qi.oninput = function () {
        var pos = qi.selectionStart; search = qi.value; render();
        var q2 = pop.querySelector('.pd-pss-q');
        if (q2) { q2.focus(); try { q2.setSelectionRange(pos, pos); } catch (e) {} }
      };
      pop.querySelectorAll('.pd-pss-proj').forEach(function (r) {
        r.onclick = function (e) { e.stopPropagation(); choose(r.dataset.proj); };
      });
      if (window.Icons) Icons.hydrate(pop);
      // Keep the selected project in view without scrolling the whole page.
      var selRow = pop.querySelector('.pd-pss-proj.sel');
    ...
    async function open() {
      ...
      await ensureData(); projs = _pdProjCache || [];
      search = '';
      render();
